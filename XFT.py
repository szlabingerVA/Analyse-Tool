...
selected_hor=option_menu(
    ...
    icons=("bezier", "bar-chart-fill", "calculator-fill", "grid-3x3-gap-fill", "card-list"),
    default_index=0,
    menu_icon="cast",
    ...
    if filesInput != None:
        ...
        try:
            ...
            with st.sidebar:
                ...
                st_echarts(optionP,height=200,width="100%")
...
if selected_hor == "ABC":
    ...
    try:
        ...
        iFile=st.selectbox("Analyse-File Auswahl",iNames, key="ABC-V")
        ...
        file=st.selectbox("Analyse-Jahr Auswahl", files, key="ABC-J", format_func=lambda x: "von " + x.name.split("_")[1])
...
if selected_hor == "Effizienz":
    ...
    file=st.selectbox("Analyse-Jahr Auswahl", files,key="EFF-J", format_func=lambda x: "von " + x.name.split("_")[1])
...
if selected_hor == "Portfolio":
    ...
    if files != None:
        ...
        try:
            #Selectbox einfügen
            file=st.selectbox("Analyse-Jahr Auswahl", files,key="PORT-J", format_func=lambda x: "von "+x.name.split("_")[1])
...
if selected_hor == "Stückliste":
    ...
    with col1:
        st.markdown("""
            ...
            """, unsafe_allow_html=True)
        st.markdown('<p class="big-font">Stückliste</p>', unsafe_allow_html=True)
    ...
    if files != None:
        ...
        try:
            ...
            dfG = dfG.sort_values('Effizienz', ascending=False)
