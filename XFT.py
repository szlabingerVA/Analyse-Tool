    ...
    if filesInput is not None:
        ...
        try:
            ...
            with st.sidebar:
                    ...
                    #Overview Auswahl
                    files=st.multiselect("Jahresauswahl", filesInput, default=filesInput, format_func=lambda x: x.name.split("_")[0]+x.name.split("_")[1])
            ...
            #         [{},{},{}]]
            # )
            # figP.add_trace(go.Bar(y=Top_A, x=MWN), row=1, col=1)
            # figP.add_trace(go.Bar(y=Top_B, x=MWN), row=1, col=2)
            # figP.add_trace(go.Bar(y=Top_C, x=MWN), row=1, col=3)
            # figP.add_trace(go.Bar(y=Medium_A, x=MWN), row=2, col=1)
            # figP.add_trace(go.Bar(y=Medium_B, x=MWN), row=2, col=2)
            # figP.add_trace(go.Bar(y=Medium_C, x=MWN), row=2, col=3)
            # figP.add_trace(go.Bar(y=Low_A, x=MWN), row=3, col=1)
            # figP.add_trace(go.Bar(y=Low_B, x=MWN), row=3, col=2)
            # figP.add_trace(go.Bar(y=Low_C, x=MWN), row=3, col=3)
            # figP.update_yaxes(showgrid=False,range=[0,1])

            # figP.update_layout(plot_bgcolor="rgba(256,256,256,0)",
            #     showlegend=False,
            #     height=700)
...
if selected_hor == "ABC-Analyse":
    ...
    try:
        ...
        file=st.selectbox("Analyse-Jahr Auswahl", files, format_func=lambda x: x.name.split("_")[1])
