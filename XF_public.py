...
selected_hor=option_menu(
    menu_title=None,
    options=("Effizienz-Analyse","ABC-Analyse","Overview","SAP-Export"),
    icons=("bar-chart-fill", "cloud-arrow-up", "bezier", "calculator-fill"),
    default_index=0,
    menu_icon="cast",
...
if selected_hor == "SAP-Export":
    ...
    with col1:
        st.markdown("""
            ...
            """, unsafe_allow_html=True)
        st.markdown('<p class="big-font">SAP-Export</p>', unsafe_allow_html=True)
    ...
    with st.expander("Upload"):
        sapfile = st.file_uploader("", accept_multiple_files=False, type=["xlsm"])

    if st.button("SAP-Export durchführen"):
        os.system('start excel.exe ' + sapfile.name)
